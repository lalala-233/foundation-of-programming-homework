...
#include <stdio.h>
#include <stdlib.h>
int64_t merge_count(int32_t* array, int32_t len) {
    if (len == 1) {
        return 0;
    }
...
int64_t merge_sort_count(int32_t* array, int32_t len) {
    int64_t count = 0;
    if (len <= 1) {
        return count;
    }
    ...
    count += merge_sort_count(left, left_len);
    count += merge_sort_count(right, right_len);
...
int32_t main(void) {
    ...
    int64_t swap_count = merge_sort_count(array, count);
    // for (int32_t i = 0; i < count; i++) {
